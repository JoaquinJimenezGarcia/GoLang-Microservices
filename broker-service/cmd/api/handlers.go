...
func (app *Config) logEventViaRPC(w http.ResponseWriter, l LogPayload) {
	...
	if err != nil {
		log.Println("Error starting dial to logger-service on RPC:", err)
		app.errorJSON(w, err)
		return
	}
	...
	if err != nil {
		log.Println("Error calling logger from RPC:", err)
		app.errorJSON(w, err)
	}
...
func (app *Config) LogViaGRPC(w http.ResponseWriter, r *http.Request) {
	...
	err := app.readJSON(w, r, &requestPayload)
	if err != nil {
		log.Println("Error logging via GRPC:", err)
		app.errorJSON(w, err)
		return
	}
	...
	if err != nil {
		log.Println("Error stablishing connection to GRPC server:", err)
		app.errorJSON(w, err)
		return
	}
	defer conn.Close()
	...
	if err != nil {
		log.Println("Error writting log via GRPC:", err)
		app.errorJSON(w, err)
		return
	}
